    ...
    app.get('/', async(req,res)=>{
        const [ groups, fields] = await connection.execute(`
                ...
                from 
                    groups 
                left join 
                    groups_users on groups.id = groups_users.group_id and groups_users.user_id = 4`,[
            req.session.user.id
        ])
        res.render('groups', {
            groups
    ...
    app.get('/:id', async(req, res)=>{
        const [group] = await connection.execute(`
            ...
            from 
                groups 
            left join 
                groups_users on groups_users.group_id = groups.id and groups_users.user_id = ? 
            where 
            groups.id = ?`,[
        ...
        const [pendings] = await connection.execute(`
                ...
                from
                    groups_users
                inner join
                    users on groups_users.user_id = users.id and 
                        groups_users.group_id = ? and
                        groups_users.role like 'pending'`,[
            req.params.id
        ])
        ...
        const [games] = await connection.execute(`
            ...
            from
                games
            left join
                guessings on games.id = guessings.game_id
                    and guessings.user_id = ? and
                    guessings.group_id = ?`,[
                        req.session.user.id,
                        req.params.id
                    ])
